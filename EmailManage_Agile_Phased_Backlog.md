...
| Phase 5 | AI-Powered Smart Scan (Ollama)          | One-click Ollama setup, AI junk detection, benchmark vs rules                | Approve AI accuracy, setup UX, and speed                        |
| Phase 6 | Safety and hardening                    | Undo, guardrails, logs, reliability improvements                             | Approve production-readiness checklist                          |
...
## Phase 5: AI-Powered Smart Scan via Ollama

### Objective

Add a fully self-contained AI feature that auto-installs Ollama and a local model, then classifies inbox emails as junk/not-junk to catch what rule-based triage misses. Benchmark AI accuracy against rule-only results. Users without Ollama click one button to get set up.

### Architecture

- **OllamaSetupService**: Detects if Ollama is installed, downloads + installs it silently if not, pulls the default model (`gemma3:4b`), reports progress via status bar.
- **OllamaClient**: HTTP client for `http://127.0.0.1:11434/api/chat`. Sends email metadata (sender, subject, body snippet, unsubscribe flag). Gets back JSON: `{ classification, confidence, reason }`.
- **AiScanService**: Fetches remaining inbox emails (post-triage), runs AI classification, compares AI vs rules, produces benchmark report.
- **AI Scan UI**: New nav item or card on Improve Model screen. Shows setup status, benchmark results grouped by domain, side-by-side "Rules say / AI says" comparison.
...
Reply with JSON only: {"classification": "JUNK"|"NOT_JUNK", "confidence": 0.0-1.0, "reason": "brief reason"}
```

### Tasks

- [ ] P5-T01 Create `OllamaSetupService` — detect Ollama installation, download installer, run silent install, verify API availability.
- [ ] P5-T02 Add "Setup AI" one-click button with progress status bar (downloading → installing → pulling model → ready).
- [ ] P5-T03 Pull default model `gemma3:4b` programmatically via Ollama API POST `/api/pull`.
- [ ] P5-T04 Create `OllamaClient` — HTTP client wrapper for `/api/chat` with timeout, retry, and structured JSON response parsing.
- [ ] P5-T05 Create `AiScanService` — fetch inbox emails, run AI classification batch, collect results.
- [ ] P5-T06 Add AI benchmark screen — show scanned count, AI-flagged junk count, rules-missed count, side-by-side comparison per email.
- [ ] P5-T07 Group benchmark results by domain with confidence bands and "Rules say / AI says" columns.
- [ ] P5-T08 Allow user to select AI-flagged items → `Delete + Learn` to teach rules from AI findings.
- [ ] P5-T09 Add "Check for Updates" button — checks Ollama version, checks for newer model tags, offers upgrade.
- [ ] P5-T10 Add AI status indicator in sidebar/settings (Installed / Not installed / Running / Model loaded).
- [ ] P5-T11 Handle Ollama not running gracefully (offer to start it, show clear error).
- [ ] P5-T12 Add scan scope setting (50 / 100 / All emails).
- [ ] P5-T13 Add per-email AI reason tooltip/detail in benchmark list.
- [ ] P5-T14 Benchmark latency logging (avg seconds per email, total scan time).

### Model Plan

- Default: `gemma3:4b` (~3 GB) — best quality/speed balance for CPU inference.
- The "Check for Updates" button checks both Ollama binary and model version.

### Demo Output

- One-click Ollama + model setup from within the app.
- AI benchmark showing gap between rules and AI classification.
- User can act on AI findings to improve rules.

### Exit Criteria

- You approve AI setup is seamless (one button).
- You approve AI accuracy on your real inbox.
- You approve speed is acceptable on your hardware.

## Phase 6: Safety, Reliability, and Release Hardening
...
- [ ] P6-T01 Add protected sender list UI.
- [ ] P6-T02 Add protected keyword list UI.
- [ ] P6-T03 Enforce hard keep-intent rules globally.
- [ ] P6-T04 Add false-positive rate monitor.
- [ ] P6-T05 Auto-degrade to `review-only` mode if risk threshold exceeded.
- [ ] P6-T06 Improve logging and diagnostics export.
- [ ] P6-T07 Optimize performance for large inboxes.
- [ ] P6-T08 Add startup health checks.
- [ ] P6-T09 Add backup/restore for pattern and state files.
- [ ] P6-T10 Add regression test suite and runbook.
- [ ] P6-T11 Add installer/update packaging draft.
- [ ] P6-T12 Prepare release candidate build.
