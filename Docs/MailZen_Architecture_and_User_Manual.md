...
- `src/EmailManage.App/EmailManage.App.csproj`: Dependencies, build metadata, product version (`2.0.0`).
...
sequenceDiagram
    ...
    VM->>TempRegistry: Build & persist TTL entries for Temp-tagged emails
...
- During `Sync Review + Relearn`, expired entries (`ExpiryUtc <= UtcNow`) are detected; the corresponding Outlook items are re-tagged from `MailZen: Temp` → `MailZen: Delete`, then removed from the registry.
...
- Transient/one-time emails use a two-tier detection model (`OutlookConnectorService`):
  - **Subject keyword match** (`IsTransientBySubjectKeyword`) — strong signal; always overrides to `MailZen: Temp` regardless of learned profile. Subject is the reliable discriminator between transient and non-transient mail from the same sender (e.g. `amazon.com` sends both "Your order shipped" and "Summer sale").
  - **Sender-prefix match** (`IsTransientBySenderPrefix`) — weak signal; checks address prefixes like `otp@`, `verify@`, `no-reply@`, `signin-noreply@`. Suppressible: if the sender is in `DoNotDeleteSenders` (user confirmed Keep via Sync), the prefix match is bypassed and the normal score applies instead.
  - `IsTransientEmail(senderEmail, subject)` combines both checks and is kept for backward compatibility.
...
4. MailZen uses read Inbox, Sent Items, and Deleted Items in the same date range as scoring context, then applies four categories to the target Inbox emails:
   ...
   - **MailZen: Temp** (orange) — transient/one-time emails (verification codes, shipping confirmations, device-auth notices, etc.) needed briefly then safe to delete. These items expire automatically based on the TTL you set in Settings > General (default: 48 hours).
5. Open Outlook and review the tagged Inbox directly:
   ...
   - filter `MailZen: Temp` to see all detected transient emails — no action needed; MailZen will auto-expire them
...
**Temp Email TTL Setting:**
In Settings > General, the "Temp Email Expiry" field controls how many hours a `MailZen: Temp` email is valid before being re-tagged as `MailZen: Delete` on the next Sync. Click "Save Defaults" to persist the value.
